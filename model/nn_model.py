...
class NN_Model(object):
    ...
    def __init__(self, input_layer, output_layer, optimizer="sgd", learning_rate=1e-3, lossfunction="CrossEntropy"):
        print("build model")
        ...
        if type(optimizer) == str:
            if optimizer not in optimizers_list:
        ...
        # 将所有相关变量放入优化器中
        self.optimizer.add_parameters(self.all_parameters)
        # 将变量加入loss中，为了计算他们的正则化，这里的正则化已经加入到参数内部了
        self.lossfunction.add_parameters(self.all_parameters)
